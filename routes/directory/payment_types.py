...
from database import Payment_types, get_db
...
async def get_users(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return templates.TemplateResponse("payment_types.html", {"request": request, "users": users_data})

@router.delete("/payment_types/{user_id}/")
async def delete_user(
    ...
    try:
        stmt = select(Payment_types).where(Payment_types.id == user_id)
        result = await db.execute(stmt)
...
@router.post("/payment_types/{user_id}/edit/")
...
async def add_user(
    ...
    try:
        metrics = Payment_types(name=name)  # Do not set the id manually
        db.add(metrics)
        await db.commit()
...
@router.post("/payment_types/{user_id}/delete/")
